...
export default function FreeTextEditor({ onExitFreeMode }) {
  ...
  const [tokens, setTokens] = useState(20); // You can fake a token count for free users if you want
  ...
  const handleLLMCorrection = async () => {
    ...
    if (wordCount > 20) {
      setCooldownEnd(new Date(new Date().getTime() + 3 * 60 * 1000));
      setError("Word limit exceeded! You are on 3 minute cooldown.");
      setText("");
      return;
    }
  ...
  useEffect(() => {
    const checkSavedCooldown = () => {
      ...
      if (savedCooldown) {
        const savedTime = new Date(savedCooldown);
        if (savedTime > new Date()) {
          setCooldownEnd(savedTime);
          setCooldownRemaining(savedTime - new Date());
        } else {
    ...
    const interval = setInterval(checkSavedCooldown, 1000); // <-- Force re-check every second in case user switches
  ...
  return (
    <div className="p-4 max-w-xl mx-auto">
      ...
      <textarea
        className={`w-full border p-2 ${cooldownRemaining !== null ? "bg-gray-200 cursor-not-allowed" : ""}`}
        rows="5"
        value={text}
        onChange={(e) => setText(e.target.value)}
        placeholder={cooldownRemaining !== null ? "You are on cooldown..." : "Enter text (max 20 words)..."}
